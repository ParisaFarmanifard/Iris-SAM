...
def infer(image_path, save_dir, pretrained_model, extension='jpg'):
    ...
    for idx, image_file in  enumerate(image_files):
        ...
        print(np.count_nonzero(pred_masks), pred_masks.shape)
        pred_masks = pred_masks[0].astype(np.uint8)#*255
        print(pred_masks.shape, pred_masks.dtype, np.unique(pred_masks))  
                
        plot_masks(input_image, pred_masks, image_path, save_dir, gt_box=input_bbox)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_model', type=str, default='checkpoints/sam_model_casia/model.pt', help='path to pretrained model')
